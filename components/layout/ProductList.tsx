...
const ProductList = ({ categoryId, categories,images,name, price, slug, id, toggleSearch }:Props) => {
  ...
  return (
    <Link onClick={()=>toggleSearch()} href={'/products/'+slug} key={id+''} className="flex gap-2 hover:bg-slate-200 cursor-pointer p-2 rounded-lg">
        <Image src={images[0]} width={120} height={120} alt={name} className="size-32 rounded-lg shadow-xl shadow-slate-200 object-cover" />
        <div>
            <p>{name}</p>
            <button className='font-bold lg:my-4 my-2 text-sm bg-slate-200 shadow-xl shadow-black/20 px-3 py-1 rounded '>${price}</button>
